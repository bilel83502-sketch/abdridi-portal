/**
 * Mégalis Bretagne — Marchés publics de Bretagne
 * https://marches.megalis.bretagne.bzh
 * Toutes les collectivités bretonnes
 *
 * Méthode : API REST JSON-LD publique /api/v2/consultations
 * Même plateforme ATEXO que Maximilien.
 * Pagination avec itemsPerPage=2 (seule valeur acceptée par le serveur).
 */

const BASE_URL = 'https://marches.megalis.bretagne.bzh';
...
/* ───── Type retour ───── */
export interface MegalisMarche {
  title: string;
  buyer: string;
  nature: string;
  department: string;
...
function mapRecord(item: any): MegalisMarche | null {
  const id = item.id;
  if (!id) return null;
  ...
  const buyer = (item.organismeDenomination || 'Mégalis Bretagne').trim();
  ...
  // Department: first from lieuxExecution array, default to 35 (Ille-et-Vilaine)
  const lieux: string[] = item.lieuxExecution || [];
  const dept = lieux.find((l: string) => /^\d{2,3}$/.test(l) || /^2[AB]$/.test(l)) || '35';
  ...
  return {
    ...
    region: deptInfo?.region || 'Bretagne',
    value,
    deadline,
    publicationDate,
    source: 'MEGALIS',
    sourceRef: `MEGALIS-${id}`,
    procedureType,
    cpvCode,
    cpvLabel: null,
...
export async function fetchMegalisRecords(options?: {
  ...
  console.log(`[MEGALIS] Fetching up to ${limit} records via API...`);
  ...
  for (let page = 1; page <= maxPages && allRecords.length < limit; page++) {
    ...
    if (!res) {
      console.warn(`[MEGALIS] Page ${page} failed (500), skipping`);
      consecutiveErrors++;
      if (consecutiveErrors >= 5) {
        console.log(`[MEGALIS] Too many consecutive errors, stopping`);
        break;
      }
      continue;
    }
    ...
    if (page === 1) {
      ...
      console.log(`[MEGALIS] ${total} consultations disponibles (${totalPages} pages)`);
    }

    if (members.length === 0) {
      console.log(`[MEGALIS] No results on page ${page}, stopping`);
      break;
    }
    ...
    console.log(`[MEGALIS] Page ${page}: ${members.length} items (total mapped: ${allRecords.length})`);
  }
  ...
  console.log(`[MEGALIS] Total records mapped: ${unique.length}`);
