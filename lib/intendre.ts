/**
 * In-Tendre — Plateforme nationale de dématérialisation des marchés publics
 * https://www.intendre.fr
 *
 * Méthode : API REST JSON-LD publique /api/v2/consultations (ATEXO probable)
 ...
 * TODO: Si non, implémenter un scraper Cheerio sur la page des consultations
 */

const BASE_URL = 'https://www.intendre.fr';
...
/* ───── Type retour ───── */
export interface IntendreMarche {
  title: string;
  buyer: string;
  nature: string;
  department: string;
...
function mapRecord(item: any): IntendreMarche | null {
  const id = item.id;
  if (!id) return null;
  ...
  const buyer = (item.organismeDenomination || 'In-Tendre').trim();
  ...
  const dept = lieux.find((l: string) => /^\d{2,3}$/.test(l) || /^2[AB]$/.test(l)) || '75';
  ...
  return {
    ...
    region: deptInfo?.region || 'Île-de-France',
    value,
    deadline,
    publicationDate,
    source: 'INTENDRE',
    sourceRef: `INTENDRE-${id}`,
    procedureType,
    cpvCode,
    cpvLabel: null,
...
/* ───── Fonction principale ───── */
export async function fetchIntendreRecords(options?: {
  limit?: number;
}): Promise<IntendreMarche[]> {
  const limit = options?.limit ?? 5000;
  const allRecords: IntendreMarche[] = [];
  const maxPages = Math.ceil(limit / PAGE_SIZE);
  let consecutiveErrors = 0;

  console.log(`[INTENDRE] Fetching up to ${limit} records via API...`);
  ...
  for (let page = 1; page <= maxPages && allRecords.length < limit; page++) {
    ...
    if (!res) {
      console.warn(`[INTENDRE] Page ${page} failed (500), skipping`);
      consecutiveErrors++;
      if (consecutiveErrors >= 5) {
        console.log(`[INTENDRE] Too many consecutive errors, stopping`);
        break;
      }
      continue;
    }
    ...
    if (page === 1) {
      ...
      console.log(`[INTENDRE] ${total} consultations disponibles (${totalPages} pages)`);
    }

    if (members.length === 0) {
      console.log(`[INTENDRE] No results on page ${page}, stopping`);
      break;
    }
    ...
    console.log(`[INTENDRE] Page ${page}: ${members.length} items (total mapped: ${allRecords.length})`);
  }
  ...
  console.log(`[INTENDRE] Total records mapped: ${unique.length}`);
