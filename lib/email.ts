...
function buildClientConfirmationHTML(data: AppointmentEmailData): string {
  return `<!DOCTYPE html>
<html lang="fr">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
<body style="margin:0;padding:0;background:#F3F4F6;font-family:Arial,Helvetica,sans-serif;">
...
export async function sendAppointmentEmails(data: AppointmentEmailData): Promise<void> {
  ...
  try {
    await resend.emails.send({
      from: FROM_EMAIL,
      to: data.clientEmail,
      subject: 'Votre demande de rendez-vous a bien \u00e9t\u00e9 re\u00e7ue \u2014 AB DRIDI',
      html: buildClientConfirmationHTML(data),
    });
    console.log('[Email] Client confirmation sent to', data.clientEmail);
  } catch (err: any) {
    console.error('[Email] Failed to send client confirmation:', err.message);
  }
}
