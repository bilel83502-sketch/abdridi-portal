/**
 * Grand Est — Marchés publics de la région Grand Est
 * https://marches-publics.grandest.fr
 *
 ...
 * TODO: Vérifier l'URL exacte de la plateforme (marches-publics.grandest.fr ou marches.grandest.fr)
 */

const BASE_URL = 'https://marches-publics.grandest.fr';
...
/* ───── Type retour ───── */
export interface GrandEstMarche {
  title: string;
  buyer: string;
  nature: string;
  department: string;
...
function mapRecord(item: any): GrandEstMarche | null {
  const id = item.id;
  if (!id) return null;
  ...
  const buyer = (item.organismeDenomination || 'Région Grand Est').trim();
...
/* ───── Fonction principale ───── */
export async function fetchGrandEstRecords(options?: {
  limit?: number;
}): Promise<GrandEstMarche[]> {
  const limit = options?.limit ?? 5000;
  const allRecords: GrandEstMarche[] = [];
  const maxPages = Math.ceil(limit / PAGE_SIZE);
  let consecutiveErrors = 0;

  console.log(`[GRANDEST] Fetching up to ${limit} records via API...`);
  ...
  for (let page = 1; page <= maxPages && allRecords.length < limit; page++) {
    ...
    if (!res) {
      console.warn(`[GRANDEST] Page ${page} failed (500), skipping`);
      consecutiveErrors++;
      if (consecutiveErrors >= 5) {
        console.log(`[GRANDEST] Too many consecutive errors, stopping`);
        break;
      }
      continue;
    }
    ...
    if (page === 1) {
      ...
      console.log(`[GRANDEST] ${total} consultations disponibles (${totalPages} pages)`);
    }

    if (members.length === 0) {
      console.log(`[GRANDEST] No results on page ${page}, stopping`);
      break;
    }
    ...
    console.log(`[GRANDEST] Page ${page}: ${members.length} items (total mapped: ${allRecords.length})`);
  }
  ...
  console.log(`[GRANDEST] Total records mapped: ${unique.length}`);
