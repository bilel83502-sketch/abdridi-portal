/**
 * Normands Achats — Marchés publics de Normandie
 * https://www.normands-achats.fr
 * Plateforme régionale Normandie
 *
 ...
 * TODO: Vérifier si l'API ATEXO est bien disponible sur cette plateforme
 */

const BASE_URL = 'https://www.normands-achats.fr';
...
/* ───── Type retour ───── */
export interface NormandsAchatsMarche {
  title: string;
  buyer: string;
  nature: string;
  department: string;
...
function mapRecord(item: any): NormandsAchatsMarche | null {
  const id = item.id;
  if (!id) return null;
  ...
  const buyer = (item.organismeDenomination || 'Normands Achats').trim();
  ...
  // Department: first from lieuxExecution array, default to 76 (Seine-Maritime)
  const lieux: string[] = item.lieuxExecution || [];
  const dept = lieux.find((l: string) => /^\d{2,3}$/.test(l) || /^2[AB]$/.test(l)) || '76';
  ...
  return {
    ...
    region: deptInfo?.region || 'Normandie',
    value,
    deadline,
    publicationDate,
    source: 'NORMANDS-ACHATS',
    sourceRef: `NORMANDS-ACHATS-${id}`,
    procedureType,
    cpvCode,
    cpvLabel: null,
...
/* ───── Fonction principale ───── */
export async function fetchNormandsAchatsRecords(options?: {
  limit?: number;
}): Promise<NormandsAchatsMarche[]> {
  const limit = options?.limit ?? 5000;
  const allRecords: NormandsAchatsMarche[] = [];
  const maxPages = Math.ceil(limit / PAGE_SIZE);
  let consecutiveErrors = 0;

  console.log(`[NORMANDS-ACHATS] Fetching up to ${limit} records via API...`);
  ...
  for (let page = 1; page <= maxPages && allRecords.length < limit; page++) {
    ...
    if (!res) {
      console.warn(`[NORMANDS-ACHATS] Page ${page} failed (500), skipping`);
      consecutiveErrors++;
      if (consecutiveErrors >= 5) {
        console.log(`[NORMANDS-ACHATS] Too many consecutive errors, stopping`);
        break;
      }
      continue;
    }
    ...
    if (page === 1) {
      ...
      console.log(`[NORMANDS-ACHATS] ${total} consultations disponibles (${totalPages} pages)`);
    }

    if (members.length === 0) {
      console.log(`[NORMANDS-ACHATS] No results on page ${page}, stopping`);
      break;
    }
    ...
    console.log(`[NORMANDS-ACHATS] Page ${page}: ${members.length} items (total mapped: ${allRecords.length})`);
  }
  ...
  console.log(`[NORMANDS-ACHATS] Total records mapped: ${unique.length}`);
