/**
 * Ternum-BFC — Marchés publics de Bourgogne-Franche-Comté
 * https://marches.ternum-bfc.fr
 * Plateforme régionale BFC
 *
 * Méthode : API REST JSON-LD publique /api/v2/consultations
 * Même plateforme ATEXO que Mégalis/Maximilien.
 * Pagination avec itemsPerPage=2 (seule valeur acceptée par le serveur).
 * TODO: Vérifier la vraie URL — peut être https://www.ternum-bfc.fr/marchespublics
 */

const BASE_URL = 'https://marches.ternum-bfc.fr';
...
/* ───── Type retour ───── */
export interface TernumBfcMarche {
  title: string;
  buyer: string;
  nature: string;
  department: string;
...
function mapRecord(item: any): TernumBfcMarche | null {
  const id = item.id;
  if (!id) return null;
  ...
  const buyer = (item.organismeDenomination || 'Ternum BFC').trim();
  ...
  // Department: first from lieuxExecution array, default to 21 (Côte-d'Or)
  const lieux: string[] = item.lieuxExecution || [];
  const dept = lieux.find((l: string) => /^\d{2,3}$/.test(l) || /^2[AB]$/.test(l)) || '21';
  ...
  return {
    ...
    region: deptInfo?.region || 'Bourgogne-Franche-Comté',
    value,
    deadline,
    publicationDate,
    source: 'TERNUM-BFC',
    sourceRef: `TERNUM-BFC-${id}`,
    procedureType,
    cpvCode,
    cpvLabel: null,
...
/* ───── Fonction principale ───── */
export async function fetchTernumBfcRecords(options?: {
  limit?: number;
}): Promise<TernumBfcMarche[]> {
  const limit = options?.limit ?? 5000;
  const allRecords: TernumBfcMarche[] = [];
  const maxPages = Math.ceil(limit / PAGE_SIZE);
  let consecutiveErrors = 0;

  console.log(`[TERNUM-BFC] Fetching up to ${limit} records via API...`);
  ...
  for (let page = 1; page <= maxPages && allRecords.length < limit; page++) {
    ...
    if (!res) {
      console.warn(`[TERNUM-BFC] Page ${page} failed (500), skipping`);
      consecutiveErrors++;
      if (consecutiveErrors >= 5) {
        console.log(`[TERNUM-BFC] Too many consecutive errors, stopping`);
        break;
      }
      continue;
    }
    ...
    if (page === 1) {
      ...
      console.log(`[TERNUM-BFC] ${total} consultations disponibles (${totalPages} pages)`);
    }

    if (members.length === 0) {
      console.log(`[TERNUM-BFC] No results on page ${page}, stopping`);
      break;
    }
    ...
    console.log(`[TERNUM-BFC] Page ${page}: ${members.length} items (total mapped: ${allRecords.length})`);
  }
  ...
  console.log(`[TERNUM-BFC] Total records mapped: ${unique.length}`);
