/**
 * PACA — Marchés publics de la région Provence-Alpes-Côte d'Azur
 * https://marchespublics-paca.fr
 *
 ...
 * TODO: Vérifier l'URL exacte de la plateforme (marchespublics-paca.fr ou marches.paca.fr)
 */

const BASE_URL = 'https://marchespublics-paca.fr';
...
function mapRecord(item: any): PACAMarche | null {
  const id = item.id;
  if (!id) return null;
  ...
  const buyer = (item.organismeDenomination || 'Région PACA').trim();
  ...
  const dept = lieux.find((l: string) => /^\d{2,3}$/.test(l) || /^2[AB]$/.test(l)) || '13';
...
/* ───── Fonction principale ───── */
export async function fetchPACARecords(options?: {
  limit?: number;
}): Promise<PACAMarche[]> {
  const limit = options?.limit ?? 5000;
  const allRecords: PACAMarche[] = [];
  const maxPages = Math.ceil(limit / PAGE_SIZE);
  let consecutiveErrors = 0;

  console.log(`[PACA] Fetching up to ${limit} records via API...`);
  ...
  for (let page = 1; page <= maxPages && allRecords.length < limit; page++) {
    ...
    if (!res) {
      console.warn(`[PACA] Page ${page} failed (500), skipping`);
      consecutiveErrors++;
      if (consecutiveErrors >= 5) {
        console.log(`[PACA] Too many consecutive errors, stopping`);
        break;
      }
      continue;
    }
    ...
    if (page === 1) {
      ...
      console.log(`[PACA] ${total} consultations disponibles (${totalPages} pages)`);
    }

    if (members.length === 0) {
      console.log(`[PACA] No results on page ${page}, stopping`);
      break;
    }
    ...
    console.log(`[PACA] Page ${page}: ${members.length} items (total mapped: ${allRecords.length})`);
  }
  ...
  console.log(`[PACA] Total records mapped: ${unique.length}`);
