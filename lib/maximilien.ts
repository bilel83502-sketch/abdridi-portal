/**
 * Maximilien — Portail des marchés publics franciliens (Île-de-France)
 * https://marches.maximilien.fr
 * 375+ acheteurs publics franciliens
 *
...
const BASE_URL = 'https://marches.maximilien.fr';
const API_URL = `${BASE_URL}/api/v2/consultations`;
const PAGE_SIZE = 2; // Le serveur n'accepte que itemsPerPage=2
const UA =
...
/* ───── Type retour ───── */
export interface MaximilienMarche {
  title: string;
  buyer: string;
  nature: string;
  department: string;
...
function mapRecord(item: any): MaximilienMarche | null {
  const id = item.id;
  if (!id) return null;
  ...
  const buyer = (item.organismeDenomination || 'Maximilien').trim();
  ...
  // Department: first from lieuxExecution array
...
export async function fetchMaximilienRecords(options?: {
  ...
  console.log(`[MAXIMILIEN] Fetching up to ${limit} records via API...`);
  ...
  for (let page = 1; page <= maxPages && allRecords.length < limit; page++) {
    ...
    if (!res) {
      console.warn(`[MAXIMILIEN] Page ${page} failed (500), skipping`);
      consecutiveErrors++;
      if (consecutiveErrors >= 5) {
        console.log(`[MAXIMILIEN] Too many consecutive errors, stopping`);
        break;
      }
      continue;
    }
    ...
    if (page === 1) {
      ...
      console.log(`[MAXIMILIEN] ${total} consultations disponibles (${totalPages} pages)`);
    }

    if (members.length === 0) {
      console.log(`[MAXIMILIEN] No results on page ${page}, stopping`);
      break;
    }
    ...
    console.log(`[MAXIMILIEN] Page ${page}: ${members.length} items (total mapped: ${allRecords.length})`);
  }
  ...
  console.log(`[MAXIMILIEN] Total records mapped: ${unique.length}`);
