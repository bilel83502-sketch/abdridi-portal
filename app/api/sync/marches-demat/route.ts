...
import { fetchMarchesDematRecords } from '@/lib/marches-demat';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-marches-demat', async () => {
      const records = await fetchMarchesDematRecords({ limit: 500 });
    ...
    console.log('[MARCHES-DEMAT] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[MARCHES-DEMAT] Sync failed:', error);
