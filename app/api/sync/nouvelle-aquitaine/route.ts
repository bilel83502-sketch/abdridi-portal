...
import { fetchNARecords } from '@/lib/nouvelle-aquitaine';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-nouvelle-aquitaine', async () => {
      const records = await fetchNARecords({ limit: 200 });
    ...
    console.log('[NA] Sync complete:', summary);
