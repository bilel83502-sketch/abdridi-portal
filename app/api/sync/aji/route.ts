...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-aji', async () => {
      const records = await fetchAjiRecords({ limit: 500 });
    ...
    console.log('[AJI] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[AJI] Sync failed:', error);
