...
import { fetchARARecords } from '@/lib/auvergne-rhone-alpes';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-auvergne-rhone-alpes', async () => {
      const records = await fetchARARecords({ limit: 200 });
    ...
    console.log('[ARA] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[ARA] Sync failed:', error);
