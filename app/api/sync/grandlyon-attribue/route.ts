...
import { fetchGrandLyonBatch } from '@/lib/grandlyon-attribue';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-grandlyon-attribue', async () => {
      const records = await fetchGrandLyonBatch({
        limit: batchSize,
    ...
    console.log('[GRANDLYON] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[GRANDLYON] Sync failed:', error);
