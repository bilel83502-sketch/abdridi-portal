...
import { fetchRennesBatch } from '@/lib/rennes-attribue';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-rennes-attribue', async () => {
      const records = await fetchRennesBatch({
        limit: batchSize,
    ...
    console.log('[RENNES] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[RENNES] Sync failed:', error);
