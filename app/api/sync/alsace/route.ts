...
import { fetchAlsaceRecords } from '@/lib/alsace';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-alsace', async () => {
      const records = await fetchAlsaceRecords({ limit: 200 });
    ...
    console.log('[ALSACE] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[ALSACE] Sync failed:', error);
