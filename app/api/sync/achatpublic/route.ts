...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-achatpublic', async () => {
      const records = await fetchAchatPublicRecords({ limit: 200 });
      ...
      // Marquer les marchés achatpublic expirés comme FERME
    ...
    console.log('[ACHATPUBLIC] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[ACHATPUBLIC] Sync failed:', error);
