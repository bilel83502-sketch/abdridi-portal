...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-francemarches', async () => {
      const records = await fetchFranceMarchesRecords({ limit: 500 });
      ...
      for (let i = 0; i < records.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          // Fallback : upserts un par un si la transaction batch échoue
    ...
    console.log('[FRANCE_MARCHES] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[FRANCE_MARCHES] Sync failed:', error);
