...
import { fetchTernumBfcRecords } from '@/lib/ternum-bfc';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-ternum-bfc', async () => {
      const records = await fetchTernumBfcRecords({ limit: 200 });
    ...
    console.log('[TERNUM-BFC] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[TERNUM-BFC] Sync failed:', error);
