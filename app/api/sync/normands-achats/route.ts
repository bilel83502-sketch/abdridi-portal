...
import { fetchNormandsAchatsRecords } from '@/lib/normands-achats';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-normands-achats', async () => {
      const records = await fetchNormandsAchatsRecords({ limit: 200 });
    ...
    console.log('[NORMANDS-ACHATS] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[NORMANDS-ACHATS] Sync failed:', error);
