...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-marches-online', async () => {
      const records = await fetchMarchesOnlineRecords({ limit: 200 });
    ...
    console.log('[MONL] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[MONL] Sync failed:', error);
