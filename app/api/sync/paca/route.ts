...
import { fetchPACARecords } from '@/lib/paca';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-paca', async () => {
      const records = await fetchPACARecords({ limit: 200 });
    ...
    console.log('[PACA] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[PACA] Sync failed:', error);
