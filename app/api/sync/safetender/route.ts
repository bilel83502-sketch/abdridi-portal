...
import { fetchSafetenderRecords } from '@/lib/safetender';
...
export async function GET(req: Request) {
  ...
  try {
    const records = await fetchSafetenderRecords({ limit: 200, maxBuyers: 50 });
    ...
    console.log('[SAFETENDER] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[SAFETENDER] Sync failed:', error);
