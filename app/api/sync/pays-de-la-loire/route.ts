...
import { fetchPDLRecords } from '@/lib/pays-de-la-loire';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-pays-de-la-loire', async () => {
      const records = await fetchPDLRecords({ limit: 200 });
    ...
    console.log('[PDL] Sync complete:', summary);
