...
import { fetchSynapseRecords } from '@/lib/synapse';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-synapse', async () => {
      const records = await fetchSynapseRecords({ limit: 200 });
    ...
    console.log('[SYNAPSE] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[SYNAPSE] Sync failed:', error);
