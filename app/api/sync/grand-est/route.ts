...
import { fetchGrandEstRecords } from '@/lib/grand-est';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-grand-est', async () => {
      const records = await fetchGrandEstRecords({ limit: 200 });
    ...
    console.log('[GRANDEST] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[GRANDEST] Sync failed:', error);
