...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-klekoon', async () => {
      const records = await fetchKlekoonRecords({ limit: 200, maxBuyers: 50 });
    ...
    console.log('[KLEKOON] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[KLEKOON] Sync failed:', error);
