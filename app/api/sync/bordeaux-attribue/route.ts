...
import { fetchBordeauxBatch } from '@/lib/bordeaux-attribue';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-bordeaux-attribue', async () => {
      const records = await fetchBordeauxBatch({
        limit: batchSize,
    ...
    console.log('[BORDEAUX] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[BORDEAUX] Sync failed:', error);
