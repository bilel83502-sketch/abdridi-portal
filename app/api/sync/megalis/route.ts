...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-megalis', async () => {
      const records = await fetchMegalisRecords({ limit: 200 });
    ...
    console.log('[MEGALIS] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[MEGALIS] Sync failed:', error);
