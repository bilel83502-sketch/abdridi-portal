...
import { fetchEuSupplyRecords } from '@/lib/eu-supply';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-eu-supply', async () => {
      const records = await fetchEuSupplyRecords({ limit: 500 });
    ...
    console.log('[EU-SUPPLY] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[EU-SUPPLY] Sync failed:', error);
