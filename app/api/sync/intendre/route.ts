...
import { fetchIntendreRecords } from '@/lib/intendre';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-intendre', async () => {
      const records = await fetchIntendreRecords({ limit: 200 });
    ...
    console.log('[INTENDRE] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[INTENDRE] Sync failed:', error);
