...
import { fetchCVLRecords } from '@/lib/centre-val-de-loire';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-centre-val-de-loire', async () => {
      const records = await fetchCVLRecords({ limit: 200 });
    ...
    console.log('[CVL] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[CVL] Sync failed:', error);
