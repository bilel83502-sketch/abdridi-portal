...
import { fetchHDFRecords } from '@/lib/hauts-de-france';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-hauts-de-france', async () => {
      const records = await fetchHDFRecords({ limit: 200 });
    ...
    console.log('[HDF] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[HDF] Sync failed:', error);
