...
import { fetchNantesBatch } from '@/lib/nantes-attribue';
...
export async function GET(req: Request) {
  ...
  try {
    const { summary } = await withCronLogging('sync-nantes-attribue', async () => {
      const records = await fetchNantesBatch({
        limit: batchSize,
    ...
    console.log('[NANTES] Sync complete:', summary);
    return NextResponse.json(summary);
  } catch (error: any) {
    console.error('[NANTES] Sync failed:', error);
