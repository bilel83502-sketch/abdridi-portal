/**
 * Script de synchronisation manuelle e-marchespublics.com (Dematis)
 * Usage : npx tsx scripts/sync-e-marchespublics.ts [--limit=200]
 */
...
async function main() {
  ...
  console.log(`\n🔄 Sync e-marchespublics.com — limit=${limit}\n`);

  const records = await fetchEMarchesPublicsRecords({ limit });
  console.log(`📥 ${records.length} annonces récupérées depuis le flux RSS\n`);
