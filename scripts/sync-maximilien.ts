/**
 * Script de synchronisation manuelle Maximilien (marches.maximilien.fr)
 * Usage : npx tsx scripts/sync-maximilien.ts [--limit=200]
 */
...
async function main() {
  ...
  console.log(`\n🔄 Sync Maximilien — limit=${limit}\n`);

  const records = await fetchMaximilienRecords({ limit });
  console.log(`📥 ${records.length} consultations récupérées depuis Maximilien\n`);
