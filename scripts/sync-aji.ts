/**
 * Script de synchronisation manuelle AJI EPLE (mapa.aji-france.com).
 * Usage : npx tsx scripts/sync-aji.ts [--limit=500]
 *
 * Charge .env.local pour DATABASE_URL (loadEnv pattern).
 */
import { readFileSync } from 'fs';
import { resolve } from 'path';
import { PrismaClient } from '@prisma/client';
import { fetchAjiRecords } from '../lib/aji';
...
async function main() {
  ...
  console.log(`\n🔄 Sync AJI EPLE — limit=${limit}\n`);

  const records = await fetchAjiRecords({ limit });
  console.log(`📥 ${records.length} AO récupérés depuis AJI EPLE\n`);
