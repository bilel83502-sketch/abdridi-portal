/**
 * Script de synchronisation manuelle BOAMP
 * Usage : npx tsx scripts/sync-boamp.ts [--limit=200] [--days=30]
 */
...
async function main() {
  ...
  const records = await fetchBoampRecords({ limit, daysBack });
  console.log(`📥 ${records.length} annonces récupérées depuis l'API BOAMP\n`);
