/**
 * Script de synchronisation manuelle Marchés Sécurisés
 * Usage : npx tsx scripts/sync-marches-securises.ts [--limit=200]
 */
...
async function main() {
  ...
  const records = await fetchMarchesSecurisesRecords({ limit });
  console.log(`📥 ${records.length} consultations récupérées depuis Marchés Sécurisés\n`);
